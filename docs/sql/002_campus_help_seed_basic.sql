...
VALUES
  (2001, 'auth:login',          '登录', 1, 'POST', '/api/auth/login', 1),
  (2101, 'order:create',        '下单', 1, 'POST', '/api/orders', 1),
  (2102, 'order:pay',           '支付', 1, 'POST', '/api/orders/*/pay', 1),
  (2103, 'order:cancel',        '取消订单', 1, 'POST', '/api/orders/*/cancel', 1),
  (2201, 'rider:grab',          '骑手抢单', 1, 'POST', '/api/rider/orders/*/grab', 1),
  (2301, 'merchant:confirm',    '商家确认接单/出餐', 1, 'POST', '/api/merchant/orders/*/confirm', 1),
