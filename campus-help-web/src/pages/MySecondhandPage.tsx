...
import { fetchMySecondhandItems } from '@/api/secondhand'
import { formatYuan } from '@/lib/utils'

export function MySecondhandPage() {
  const { data, isLoading, error } = useQuery({
    queryKey: ['secondhand', 'mine'],
    queryFn: fetchMySecondhandItems,
  })
  ...
  return (
    <div className="mx-auto max-w-lg">
      <div className="mb-6 flex items-center gap-3">
        ...
        <div>
          <h1 className="text-lg font-semibold text-slate-900">我发布的</h1>
          <p className="text-xs text-slate-500">含在售、已下架、已售等状态</p>
        </div>
      </div>
      ...
      <ul className="space-y-3">
        {data?.map((it) => (
          <li key={it.id}>
            <Link
              to={`/secondhand/${it.id}`}
              className="flex gap-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm transition hover:border-amber-300"
            >
      ...
      {!data?.length && (
        <p className="py-12 text-center text-sm text-slate-500">
          还没有发布过闲置，去{' '}
          <Link to="/secondhand/new" className="text-amber-700 underline">
            发布
          </Link>
        </p>
      )}
